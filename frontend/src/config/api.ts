// Centralized API Base URL Configuration for Vercel / Production / Local / Mobile LAN
const getDynamicHost = () => {
  if (import.meta.env.VITE_API_HOST) return import.meta.env.VITE_API_HOST;
  if (import.meta.env.VITE_API_URL) {
    return import.meta.env.VITE_API_URL.replace(/\/api\/v1\/?$/, '');
  }
  // When viewing from a mobile phone connected over Wi-Fi / LAN (e.g., http://192.168.x.x:5003)
  if (
